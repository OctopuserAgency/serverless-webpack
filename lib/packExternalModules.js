...
function flatDependencies(externals, graph) {
  let result = [];
  const modifiedGraph = graph;
  const dependencies = _.keys(graph.dependencies);
  const filteredMainDependencies = _.filter(dependencies, key => _.includes(externals, key));
...
module.exports = {
  ...
  packExternalModules(stats) {
    ...
    return new BbPromise((resolve, reject) => fs.access(path.join(process.cwd(), 'node_modules'), (err) => {
      if(err)  {
        return reject(new Error('There isn\'t node_modules or i\'ts not readable. Maybe you should run \'npm install\''));
      }
      return resolve();
    }))
    ...
    .then(dependencyGraph => {
      ...
      return new BbPromise((resolve, reject) => {
        const start = _.now();
        const moduleCopyPromises = _.map(compositeModules, moduleName =>
        ...
        BbPromise.all(moduleCopyPromises)
        .then(() => {
